...
class SocketClient
{
    ...
    public function sendData($data, $type = 'text', $masked = true)
    {
        ...
        //while ($buff = socket_read($this->_Socket, 1024, PHP_BINARY_READ)) {
        //    var_dump($buff);
    ...
    public function connect($host, $port, $path, $origin = false)
    {
        ...
        $connection = socket_connect($this->_Socket, $host, $port);
        //socket_set_timeout($this->_Socket, 0, 10000);
        socket_write($this->_Socket, $header); 
        $response = socket_read($this->_Socket, 1500, PHP_BINARY_READ);        

        if (!preg_match('/Sec-WebSocket-Accept:\s(.*)/', $response, $matches)) {
            return false;
        }
